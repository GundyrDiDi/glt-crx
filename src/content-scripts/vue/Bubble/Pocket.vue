<template>
  <div class="sniff-crx-bubble-pocket flex-col-bwn rel">
    <slot></slot>
    <div v-if="!list.length" class="abs-wrap flex-cen" style="padding: 100px 10px; color: #aaa; text-align:center">
    ...
    <transition-group tag="div" name="list" class="sniff-crx-bubble-pocket-list">
      <div v-for="(v, i) in list" :key="v.time + v.productSpecification" class="sniff-crx-bubble-pocket-item flex rel">
        <div>
        ...
        <div class="sniff-crx-bubble-pocket-desc">
    ...
    <div class="sniff-crx-bubble-pocket-bottom flex-ter-bwn">
      <div>
        <span>{{ $t("数量") }}: </span>
        <span class="sniff-crx-bubble-pocket-num">{{ list.reduce((acc, v) => acc + v.quantity, 0) }}</span>
      </div>
      <div>
